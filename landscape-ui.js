...
export function initLandscape(){
 ...
 const toggle=document.createElement('button');toggle.id='landscape-menu';toggle.className='secondary';toggle.innerHTML=icon('menu');toggle.setAttribute('aria-label','Открыть меню');toggle.setAttribute('aria-expanded','false');aside.id='game-navigation';toggle.setAttribute('aria-controls',aside.id);
 ...
 const change=value=>{open=value&&media.matches;document.body.classList.toggle('landscape-menu-open',open);toggle.setAttribute('aria-expanded',String(open));toggle.setAttribute('aria-label',open?'Закрыть меню':'Открыть меню');toggle.innerHTML=icon(open?'close':'menu');aside.inert=media.matches&&!open;main.inert=open;if(open)aside.querySelector('nav button.active')?.focus();};
