(() => {
  ...
  var init_balance = __esm({
    "balance.js"() {
      ...
      xpForLevel = (level) => Math.round(240 * (level - 1) + 90 * (level - 1) * (level - 2));
      playerLevel = (s) => {
        let level = 1;
      ...
      levelProgress = (s) => {
        const level = playerLevel(s), start2 = xpForLevel(level), next = xpForLevel(level + 1);
      ...
      enemyStats = (map, wave, type) => ({ hp: (type === "boss" ? 230 : type === "tank" ? 86 : type === "runner" ? 28 : 40) * (1 + map * 0.32) * (1 + (wave - 1) * 0.15), speed: type === "boss" ? 34 : type === "runner" ? 95 : type === "tank" ? 28 : 47, damage: (type === "boss" ? 23 : type === "tank" ? 17 : 10) * (1 + map * 0.16) });
      ARMOR = [
  ...
  function setAppearance(save2) {
    var _a2;
    appearance = { weapon: save2.weapon || 0, armor: (_a2 = save2.armorTier) != null ? _a2 : save2.armor > 0 ? 1 : 0 };
  }
  ...
  async function loadArt() {
    ...
    }))), loadSprite("assets/characters.png").then((c) => spriteAtlas = c), loadSprite("assets/equipment.png").then((c) => equipmentAtlas = c), loadSprite("assets/armor-tiers.png").then((c) => armorAtlas = c), loadSprite("assets/items.png").then((c) => itemAtlas = c), loadSprite("assets/weapons-loot.png").then((c) => weaponAtlas = c)]);
  }
  ...
  function drawWeapon(canvas2, index) {
    if (!weaponAtlas) return;
    const ctx = canvas2.getContext("2d");
    ctx.clearRect(0, 0, canvas2.width, canvas2.height);
    ctx.imageSmoothingEnabled = false;
    ctx.drawImage(weaponAtlas, index * 512, 0, 512, 512, 0, 0, canvas2.width, canvas2.height);
  }
  ...
  function refresh() {
    ...
    $("#xp-caption").textContent = Math.floor(progress.current) + " / " + progress.required + " XP";
  ...
  function spawn(type, i) {
    ...
    let st = enemyStats(r.map, Math.min(r.wave, 3), type);
